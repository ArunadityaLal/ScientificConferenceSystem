...
export default function EventDetailsPage() {
  ...
  const totalSessions =
    sessionsData?.data?.stats?.totalSessions || sessions.length;
  const totalRegistrations =
    registrationsData?.data?.stats?.totalRegistrations || registrations.length;
  ...
  const confirmedFaculty = faculty.filter(
    (f: any) => f.status === "CONFIRMED"
  ).length;

  // ✅ FIXED: Safe event status calculations
  const isEventPast =
    event && event.endDate ? new Date(event.endDate) < new Date() : false;
  const isEventActive =
      ...
      <div className="space-y-6 p-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            ...
            <ExportButton eventId={eventId} variant="outline" size="default" />
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {event.maxParticipants
                  ? `${Math.round(
                      (approvedRegistrations / event.maxParticipants) * 100
                    )}%`
                  : "N/A"}
              </div>
              <p className="text-xs text-muted-foreground">
                  ...
                  <div>
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex items-center">
                        ...
                        <span>
                          Expected: {event.maxParticipants || "Not specified"}{" "}
                          attendees
                        </span>
                      </div>
                    </div>
                ...
                {sessionsLoading ? (
                  ...
                  <div className="space-y-4">
                    {sessions.slice(0, 3).map((session: any) => {
                      ...
                      const isSessionPast = isValidDate
                        ? sessionDate < new Date()
                        : false;
                      const isSessionToday = isValidDate
